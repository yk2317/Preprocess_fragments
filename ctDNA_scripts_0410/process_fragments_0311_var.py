...
def save_master(input_file, output_dir, genome):
    base_name = input_file.split('/')[-1].replace('.bed', '')
    output_file = f"{output_dir}/{base_name}_varmaster.txt"
    ...
    with open(input_file, 'r') as f:
        for line in f:
            chr, start, end, name, strand, length, pos, ref, alt, dist_start, dist_end, clipped, gc_content, mapq, mbq, consensus_min_depth, consensus_error_rate = line.strip().split('\t')
            ...
            if "N" not in motif:
                processed_line = [chr, str(start), str(end), name, gc_content, motif, pos, ref, alt, dist_start, dist_end, clipped, mapq, mbq, consensus_min_depth, consensus_error_rate]
...
def save_count(processed_lines, output_dir, is_positive_strand):
...
if __name__ == "__main__":
    ...
    save_count(processed_lines, output_dir, is_positive_strand)
