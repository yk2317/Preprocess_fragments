...
normalize <- function(sub, dataset, strand){

  if(dataset == "MSK"){
    if(strand=="pos"){
        ref <- read.csv("/n/data1/hms/dbmi/park/ctDNA_loci_project/fragmentomics/shared_tools/panel_4mer_counts/pan_v2_Manifest.wo_iSNP.wo_decoy.merged.hg38_forward_4mer_counts.csv")
    }else if(strand=="neg"){
        ref <- read.csv("/n/data1/hms/dbmi/park/ctDNA_loci_project/fragmentomics/shared_tools/panel_4mer_counts/pan_v2_Manifest.wo_iSNP.wo_decoy.merged.hg38_reverse_4mer_counts.csv")
    }
  }
  if(dataset == "Gerberg"){
    if(strand=="pos"){
        ref <- read.csv("/n/data1/hms/dbmi/park/ctDNA_loci_project/fragmentomics/shared_tools/panel_4mer_counts/BroadPanCancer2019.hg38_forward_4mer_counts.csv")
    }else if(strand=="neg"){
        ref <- read.csv("/n/data1/hms/dbmi/park/ctDNA_loci_project/fragmentomics/shared_tools/panel_4mer_counts/BroadPanCancer2019.hg38_reverse_4mer_counts.csv")
    }    
  }
  ...
  for(i in 1:3){
    ...
    if(param1){
      if(i != 1){
        ...
        pos_norm <- normalize(pos, dataset, "pos")

        neg <- comb[,str_detect(colnames(comb), "neg")]
        neg_norm <- normalize(neg, dataset, "neg")
